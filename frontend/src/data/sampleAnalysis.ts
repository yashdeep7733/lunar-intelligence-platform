...
export type AnalysisResponse = {
  ...
  images: {
    ...
    landing_suitability_map: string;
    charts: {
      crater_size_distribution: string;
      hazard_breakdown: string;
      crater_density_distribution: string;
    };
  };
  hazard: { score: number; level: "Low" | "Medium" | "High" | "Critical" | string };
  surface: { texture: string; crater_density: string; coverage_percent: number };
  statistics: {
    total_craters: number;
    largest_crater_px: number;
    smallest_crater_px: number;
    average_crater_diameter_px: number;
    median_crater_diameter_px: number;
  };
  landing_zone: {
    status: string;
    region: string;
    ...
    estimated_area_px: number;
    reason: string[];
  };
  explainable_ai: { summary: string };
  mission_summary: { overall_assessment: string; recommendation: string };
};
