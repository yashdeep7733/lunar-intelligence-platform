...
import { useAnalyzeImage } from "@/hooks/useLipData";
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: (accepted, rejected) => {
      ...
      if (accepted.length) {
        ...
        // Sends the image through the API layer (mock today, real backend later).
        analyze.mutate(accepted[0], {
          onSuccess: (result) =>
            toast.success("Analysis complete", {
              description: `${result.statistics.total_craters} craters detected — hazard ${result.hazard.score}/100.`,
            }),
          onError: (error) => toast.error("Analysis failed", { description: error.message }),
        });
        toast.success(`${accepted.length} image(s) added`, { description: "Running analysis…" });
      }
    },
  });
